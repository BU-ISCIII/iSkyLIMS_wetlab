...
def run ():
    ...
    with open ('part1_sampleInProject_migration_data.csv', 'w') as fh:
        for sample in samples_to_update :
            ...
            run_pk =  project_obj.runprocess_id.pk
            fh.write(str(sample.pk) + ',' + str(run_pk) + '\n')

    with open ('part1.1_sampleInProject_migration_data.csv', 'w') as fh:
        for sample in samples_to_update :
            project_obj = sample.project_id
            fh.write(str(sample.pk) + ',' + str(project_obj.pk) + '\n')
